...
public class According_course extends JFrame{
	private JTable table;
	private MytableModel1 tablemodel;
	public According_course(String serch) {
	...
	private MytableModel1 getModel(String serch) {
		...
		try {//链接数据库，执行查询语句
			...
			ResultSet rs=dbcon.executeQuery("select course.*,student.*,sc.academicyear ,sc.dailygrade ,sc.finalgrade ,sc.relearn ,sc.relearngrade ,sc.totalgrade"
					+ " from student,course,sc where student.sno=sc.sno and course.cno=sc.cno and cname="+"'"+serch+"'");
			//获取查询结果的列名，填充表格模型列
